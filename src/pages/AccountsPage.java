...
//			narrationTxt.click();
		...
		try{
			...
			if(!totalDebitAmt.getAttribute("value").equals(totalCreditAmt.getAttribute("value"))){
				if(totalDebit > totalCredit){
					...
					for(int j=0;j<creditBillValue.size();j++){
						sActAmt = creditBillValue.get(j).getAttribute("value");
						if(Double.parseDouble(sActAmt) > 0){
							creditBillValue.get(j).sendKeys(Keys.CONTROL+"a");
							creditBillValue.get(j).sendKeys(Keys.DELETE);
							creditBillValue.get(j).sendKeys(Double.toString((Double.parseDouble(sActAmt)+roundOff)));
							break;
						}
					}
				}else{
					...
					double roundOff = Math.round(dDiff * 100.0) / 100.0;
					for(int j=0;j<debitBillValue.size();j++){
						sActAmt = debitBillValue.get(j).getAttribute("value");
						if(Double.parseDouble(sActAmt) > 0){
							debitBillValue.get(j).sendKeys(Keys.CONTROL+"a");
							debitBillValue.get(j).sendKeys(Keys.DELETE);
							debitBillValue.get(j).sendKeys(Double.toString((Double.parseDouble(sActAmt)+roundOff)));
							break;
						}
					}
				}
	...
	public boolean verifyBillNo_AfterBillDelete(){
		...
		try{
		    ...
		    Thread.sleep(Constant.Min_Sleep);
		    for(int i=0;i<ledgerNameList_BillPopup.size();i++){
		    	for(int j=0;j<sLedger.size();j++){
			    	if(sLedger.get(j).contains(ledgerNameList_BillPopup.get(i).getText())){
			    		ledgerNameList_BillPopup.get(i).click();
			    		 for(int k=0;k<billNoList_BillsTableAccts.size();k++){
			 		    	if(!sBillNo.contains(billNoList_BillsTableAccts.get(k).getText())){
			 		    		flag = true;
			 		    		break;
			 		    	}
			 		    }
	...
	public boolean mandatoryFieldsBillPopup(int iRowNo, String sheet){
		...
		try{
			...
			for(int i=0;i<billAddBtn.size();i++){
				if(billAddBtn.get(i).isDisplayed()){
					billAddBtn.get(i).click();
					Thread.sleep(Constant.Min_Sleep);
					break;
				}
			}
	...
	public boolean accountDashboardValidation(){
		boolean flag = false;
		List<BigDecimal> RecBal = new ArrayList<BigDecimal>();
		List<BigDecimal> overdueRec = new ArrayList<BigDecimal>();
		List<BigDecimal> payBal = new ArrayList<BigDecimal>();
		List<BigDecimal> overduePay = new ArrayList<BigDecimal>();
		List<BigDecimal> cashBal = new ArrayList<BigDecimal>();
		List<BigDecimal> bankBal = new ArrayList<BigDecimal>();
		List<BigDecimal> salesBal = new ArrayList<BigDecimal>();
	...
	public boolean validateBRS_AddingVoucher(int iRowNo, String sheet){
		...
		try{
			...
			for(int i=0;i<voucherNo.size();i++){
				if(voucherNo.get(i).getText().contains(sVoucherNo)){
					flag = true;
					break;
				}
			}
