...
public class StorePage {
	...
	/// @FindBy(xpath="//span[@class='btn-new-item-label']") WebElement addIndentIcon; //-Gayathri added
	
	@FindBy(xpath=".//a[@data-original-title='Create New Indent']") 
	private WebElement addIndentIcon;  
	...
	@FindBy(xpath="//div[@class='datepicker-days']//th[@class='next'][normalize-space()='»']") //Gayathri added
	// @FindBy(xpath="//div[@class='datepicker-days']//th[@class='next'][contains(text(),'�')]") -Gayathri command
		...
		try{
			if(!(sMaterial == null)){
				...
				for(int i=0;i<sMaterialList.length;i++){
					...
					Thread.sleep(Constant.Min_Sleep);
					driver.findElement(By.xpath(".//ul/li/a[contains(text(),'"+sMaterialList[i].trim()+"')]")).click();
					Thread.sleep(Constant.Min_Sleep);
					...
					addMaterial.click();
					Thread.sleep(Constant.Min_Sleep);
					if(driver.findElement(By.xpath(".//div[contains(text(),'"+sMaterialList[i].trim()+"')]")).isDisplayed()){
						flag = true;
					}
				}
			}else{
...
//			}
				...
				if(last30Days.isDisplayed()){
					...
					if(currentDate.equals(dateTo) && date30.equals(dateFrom)){
						...
						for(int i=0;i<DateList.size();i++){
							Date date2 = dateFormat.parse(DateList.get(i).getText());
	...
	public boolean addindentpo_spq(int iRowNo, String sheet){
		...
		try{
			...
			Thread.sleep(Constant.Min_Sleep);
			if(Constant.isElementPresent(By.id("btn-sa-confirm")) == true){
				okBtn.click();
				Thread.sleep(Constant.Min_Sleep);
				indentPurchaseBtn.click();
				Thread.sleep(Constant.Min_Sleep);
		...
		try{
			...
			if(flag1 == true && flag2 == true && flag3 == true && flag4 == true && flag5 == true){
				...
				if(sReceivedAgainst.contains("Job In") && sPoMandatory.contains("Without Po") && sCreateOA.contains("Yes")){
					Constant.waitFor(ExpectedConditions.visibilityOf(continueBtn), "Failed to load oa link warning popup");
					if(addOAMaterial(iRowNo,sheet) == true){
						continueBtn.click();
						Thread.sleep(Constant.Max_Sleep);
					}
				}
				...
				Constant.waitForPageLoad();
				if(Constant.isElementPresent(By.xpath(".//h2/span[text()='Saved Successfully']/following::p/b[2]")) == true){
		...
		try{
			if(!sReceivedAs.contains("Goods")){
				...
				for(int i=0;i<MaterialListPartyDcNo.size();i++){
					MaterialListPartyDcNo.get(i).click();
					String[] sMaterialList = MaterialListPartyDcNo.get(i).getText().split("\\s+");
		...
		try{
			...
			if(sIssueNo == null){
				...
				if(issueNoYrOptionDrpDwnList.size() > 0){
					for(int i=0;i<issueNoYrOptionDrpDwnList.size();i++){
						issueNoYrOptionDrpDwnList.get(i).click();
						Thread.sleep(Constant.Min_Sleep);
					}
				}
				Thread.sleep(Constant.Max_Sleep);
	...
	//					sPoNo = sPoNo.substring(0, 7)+sPoNo.substring(9,iCount);
			...
			if(!sReceivedAgainst.contains("Job In")){
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN view page");
			}
		}catch(Exception e){
	...
	public boolean validateOANowithMat(String sOaNo){
		boolean flag = false;
		try{
			for(int i=0;i<oaDropDwnEditPge.size();i++){
				if(oaDropDwnEditPge.get(i).getText().contains(sOaNo)){
					flag = true;
				}else{
					break;
				}
	...
	public String selectDCPartyNo(int iRowNo, String sheet){
		...
		try{
			...
			if(sPoMandatory.contains("With Po")){
				for(int i=0;i<partyDcNo.size();i++){
					partyDcNo.get(i).click();
					Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN view page");
					partyDcDropDwn.click();
	...
	public boolean addStockMaterial(int iRowNo, String sheet){
		...
		try{
			for(int i=0;i<sMaterialList.length;i++){
				...
				if(checkElement == true && driver.findElement(By.id("btn-sa-confirm")).isDisplayed()){
					okButton.click();
					Thread.sleep(Constant.Min_Sleep);
			...
			for(int i=0;i<materialsAddedInGRN.size();i++){
				...
				if(sPoMandatory.contains("With and Without Po")){
					if(materialsAddedInGRN.get(i).getText().contains(sMaterialList[j].trim())){
						...
						if(flag1 == true && flag2 == true && flag3 && flag4 ==true){
							flag = true;
						}
					}
				}
	...
	public boolean validateGrandTotalValue_Goods(){
		...
		ArrayList<String> sTotal = new ArrayList<String>();
		ArrayList<String> sNetCGST = new ArrayList<String>();
		ArrayList<String> sNetSGST = new ArrayList<String>();
		ArrayList<String> sNetIGST = new ArrayList<String>();
		List<Double> arrayOfTax = new ArrayList<Double>();
		List<Double> arrayOfTotal = new ArrayList<Double>();
		List<Double> arrayOfCgst = new ArrayList<Double>();
		List<Double> arrayOfSGST = new ArrayList<Double>();
		List<Double> arrayOfIGST = new ArrayList<Double>();
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				if(sReceivedAgainst.contains("Issues")){
					...
					Thread.sleep(Constant.Min_Sleep);
					for(int i=0;i<sMaterialList.length;i++){
						if(MSLWarning.getText().contains(sMaterialList[i])){
							flag = true;
						}else{
							flag = false;
						}
	...
	public boolean validateMSLWarning_MatReceived(int iRowNo, String sheet){
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				if(sReceivedAgainst.contains("Issues")){
					for(int i=0;i<acceptedQtyList.size();i++){
						String sValue = Double.toString(Double.parseDouble(acceptedQtyList.get(i).getAttribute("value"))-1);
						acceptedQtyList.get(i).sendKeys(Keys.CONTROL + "a");
						acceptedQtyList.get(i).sendKeys(Keys.DELETE);
						acceptedQtyList.get(i).sendKeys(sValue);
					}
					remarksTxtField.click();
					UpdateBtn.click();
					Thread.sleep(Constant.Min_Sleep);
	...
	public boolean updateQty_OnlyReceived(int iRowNo, String sheet){
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				...
				issueNoDrpDwn.click();
				Thread.sleep(Constant.Min_Sleep);
				for(int j=0;j<selectedIssueNo.size();j++){
					js.executeScript("arguments[0].scrollIntoView(true);", selectedIssueNo.get(j));
					sIssueNo = selectedIssueNo.get(j).getText();
				}
			}
			flag = validateUpdateGRN(iRowNo, sheet, iGRNCount, sIssueNo);
		}catch(Exception e){
		...
		try{
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				...
				if(sIssueNo == null){
					...
					Thread.sleep(Constant.Min_Sleep);
					if(issueNoYrOptionDrpDwnList.size() > 0){
						for(int i=0;i<selectedIssueNoYrOp.size();i++){
							selectedIssueNoYrOp.get(i).click();
							Thread.sleep(Constant.Min_Sleep);
						}
						for(int i=0;i<issueNoYrOptionDrpDwnList.size();i++){
							issueNoYrOptionDrpDwnList.get(i).click();
	...
	public boolean verifyPendingQtyOfIssue(int iRowNo, String sheet, String sIssueNo){
		...
		try{
			...
			Thread.sleep(Constant.Min_Sleep);
			for(int i=0;i<pendingReturnedQtyList.size();i++){
				if(pendingReturnedQtyList.get(i).getText().contains(Integer.toString(Integer.parseInt(sQty)+1))){
					flag = true;
				}else{
					flag = false;
					break;
	...
	public boolean updateIRN_ReduceMatQty(int iRowNo, String sheet, String sIssueNo){
		...
		try{
			...
			grnList.get(iGRNCount-1).click();
			Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN view page");
			if(updateQty(iRowNo, sheet, sGRNNo) == true){
				UpdateBtn.click();
				Thread.sleep(Constant.Min_Sleep);
				okButton.click();
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN view page");
	...
	public boolean updateIRN_ChangeIssueNo(int iRowNo, String sheet, String sStatus){
		...
		ArrayList<String> sSelectedIssueNo = new ArrayList<String>();
		try{
			Constant.waitFor(ExpectedConditions.elementToBeClickable(filterText), "Failed to load GRN view page");
			navigate.showAllEntries();
			Thread.sleep(Constant.Min_Sleep);
	...
	public boolean addTax(int iRowNo,String sheet) throws Exception{
		...
		try{
			...
			addTaxBtn.click();
			Thread.sleep(Constant.Min_Sleep);
			for(int i=0;i<addedTax.size();i++){
				if(addedTax.get(i).getText().contains(sTax)){
					flag = true;
					break;
				}
			}
	...
	public boolean validateGRNUpdate(int iRowNo, String sheet, String sIssueNo){
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
 				...
 				if(sReceivedAgainst.contains("Delivery Challan")){
	 				for(int i=0;i<dcMaterialGridList.size();i++){
	 					sPONO = dcMaterialGridList.get(i).getText();
	 					if(!sPONO.isEmpty()){
	 						break;
	 					}
	 				}
 				...
 				if(sReceivedAgainst.contains("Job In")){
	 				for(int i=0;i<oaNoDropDwnMatGridList.size();i++){
	 					sPONO = oaNoDropDwnMatGridList.get(i).getText();
...
//				Runtime.getRuntime().exec("E:\\ExpenseBillUpload.exe");//execute AutoIT to upload import file
				...
				if(flag == true && matReceivedTab.isDisplayed()){
					matReceivedTab.click();
				}
		...
		try{
			...
			if(!(sReceivedAgainst.contains("Delivery Challan") || sReceivedAgainst.contains("Others") || sReceivedAgainst.contains("Issues") || sPoMandatory.equals("Without Po")
				...
				if(sReceivedAs.contains("Goods")){
					...
					flag3 = addSGSTTax(0,iRowNo,sheet);
					flag4 = addIGSTTax(0,iRowNo,sheet);
					for(int i=0;i<grnHSNCodeList.size();i++){
						if(grnHSNCodeList.get(i).getAttribute("value").isEmpty() || grnHSNCodeList.get(i).getAttribute("value").matches(".*[a-zA-Z]+.*")){
							grnHSNCodeList.get(i).sendKeys(Keys.CONTROL+"a");
							grnHSNCodeList.get(i).sendKeys(Keys.DELETE);
							grnHSNCodeList.get(i).sendKeys(sDate);
			...
			if(sReceivedAgainst.contains("Others") || sReceivedAgainst.contains("Issues") || sPoMandatory.contains("Without Po")){
				if(!sReceivedAs.contains("Goods")){
					...
					if(!sPoMandatory.contains("Without Po")){
						for(int i=0;i<receivedQtyList.size();i++){
							receivedQtyList.get(i).clear();
							receivedQtyList.get(i).sendKeys("550");
						}
					}
				}
			}
	...
	public boolean approveGRN(int iRowNo, String sheet, String sIssueNo){
		...
		try{
			...
			Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN edit page");
			Thread.sleep(Constant.Min_Sleep);
			js.executeScript("window.scrollTo(0, document.body.scrollHeight)");
			Thread.sleep(Constant.Min_Sleep);
			...
			if(grnPDF.isDisplayed()){
				if(remarksCount.getText().contains("2")){
					...
					for(int i=0;i<availableRemarks.size();i++){
						if(availableRemarks.get(i).getText().contains("Approve GRN automation"+sDate)){
							flag = true;
							break;
						}
					}
...
//				okButton.click();
	...
	public boolean validateOANoNotAvail(int iRowNo, String sheet, String sOANo){
		...
		try{
			...
			if(Constant.isElementPresent(By.xpath("//label[contains(text(),'"+sOANo+"')]")) == false){
				flag = true;
			}
		}catch(Exception e){
	...
	public boolean validateMaterialIssueReport(int iRowNo, String sheet,String sGRNNo){
		...
		try{
			...
			Thread.sleep(Constant.Min_Sleep);
			for(int i=0;i<receiptNoColumn.size();i++){
				js.executeScript("arguments[0].scrollIntoView(true);",receiptNoColumn.get(i));
				if(receiptNoColumn.get(i).getText().contains(sGRNNo)){
					flag = true;
					break;
				}
			}
...
//			}
		...
		try{
			...
			if (sSearchIn != null) {
				...
				for (int i = 0; i < sSplitData.length; i++) {
					if (!(sSplitData[i] == null) || !(sSplitData[i].length() == 0)) {
						Result.add(sSplitData[i].trim());
					}
				}
			}
	...
	public boolean closingStockColumnValidation(int iRowNo, String sheet) throws Exception{
		...
		try{
			...
			reportView.click();
			Constant.waitFor(ExpectedConditions.visibilityOfAllElements(closingStockColumnNames), "Failed to load indent pending report");
	...
	public boolean shortageListColumnValidation(int iRowNo, String sheet) throws Exception{
		...
		try{
			...
			for(int i=0; i<shortageReportColumns.size();i++){
				array.add(shortageReportColumns.get(i).getText());
				if(shortageReportColumns.get(i).getText().contains("SHORTAGE")){
					break;
				}
			}
			for(String s : array){
	...
	public String shortClose(){
		List<Double> poQty = new ArrayList<Double>();
		List<Double> indentQty = new ArrayList<Double>();
	...
	public boolean multipleMaterial(int iRowNo) throws Exception{
		 ...
		 try{
			...
			for(int i=0;i<material.size();i++){
				materialReqTxt.sendKeys(material.get(i));
				Thread.sleep(Constant.Min_Sleep);  
				driver.findElement(By.xpath(".//a[text()='"+material.get(i)+"']")).click();
				Thread.sleep(Constant.Min_Sleep); 
				quantityTxt.sendKeys(sQuantity);
				addMaterial.click();
				if(driver.findElement(By.xpath(".//div[contains(text(),'"+material.get(i)+"')]")).isDisplayed()){
					Constant.log.info(material.get(i) +"Material is added successfully");
				}else{
					System.err.println(material.get(i) +"Material is not added");
				}	
				Thread.sleep(Constant.Min_Sleep);
			}	
			indentRemark.sendKeys("Remark"+sDate);
	...
	public boolean draftIRNDetailsValidate(int iRowNo, String sheet){
		...
		String sMaterial = xls.get(iRowNo).get("Material");
		String sFaulty = xls.get(iRowNo).get("Faulty");
		List<String> sMatList = new ArrayList<String>();
		...
		try{
			...
			if(returnMaterialPopup.isDisplayed()){
				...
				if(sFaulty.contains("Yes")){
					for(int i=0;i<sMatList.size();i++){
						if(sMatList.get(i).contains("Faulty")){
							flag = true;
							break;
						}else{
							flag = false;
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				...
				if(driver.findElement(By.xpath("//td/a[contains(text(),'"+sGRNNo+"')]/following::td[9]")).getText().contains("Approved")){
					...
					Thread.sleep(Constant.Min_Sleep);
					navigate.showAllEntries();
					for(int i=0;i<grnNoList.size();i++){
	...
	public boolean createPO_IndentPo(int iRowNo, String sheet){
		...
		try{
			...
			if(driver.getPageSource().contains(sPODraftNo[0]) && iActPONumber > iPONumber && flag1 ==true){
				flag = true;
			}
		}catch(Exception e){
	...
	public String createPO(int iRowNo, String sheet){
		...
		String sMaterial = xls.get(iRowNo).get("Material");
		String[] sMaterialList = sMaterial.split(",");
 		String sPODraftNo = null; List<String> sSupplierList = new ArrayList<String>();
		try{
			if(indentPurchaseBtn.isDisplayed()){
				...
				for(int i=0;i<purchaseCheckboxes.size();i++){
					purchaseCheckboxes.get(i).click();
				}
				for(int i=0;i<jobOrderCheckboxes.size();i++){
					jobOrderCheckboxes.get(i).click();
				}
				for(int i=0;i<sMaterialList.length;i++){
	...
	public boolean createPO_ExistingIndent(int iRowNo, String sheet){
		...
		try{
			...
			navigate.showAllEntries();
			int iActPONumber = Integer.parseInt(poVisibleRecordCount.getText().substring(14).trim());
			if(driver.getPageSource().contains(sPODraftNo) && iActPONumber > iPONumber && flag1 ==true){
				flag = true;
			}
	...
	public boolean amendIR_AddingMat(int iRowNo, String sheet){
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sGRNNo)){
				...
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(okButton), "Failed to load success poppu");
				Thread.sleep(Constant.Min_Sleep);
				okButton.click();
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(grnMaterialTable), "Failed to load GRN edit page");
				js.executeScript("window.scrollBy(0,-5000)");
				backBtn.click();
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(filterText), "Failed to load GRN view page");
			}
			navigate.navigateToStore();
			Thread.sleep(Constant.Min_Sleep);
			navigate.navigateToIssue();
	...
	public boolean verifyItemsInvoiced_DraftRejected(int iRowNo, String sheet){
		...
		try{
			...
			if(grnDraft_EditPage.getText().contains(sDraftNo)){
				...
				approveRejectBtn.click();
				Constant.waitFor(ExpectedConditions.visibilityOfAllElements(rejectGRNBtn), "Failed to load GRN edit page");
				if(grnPDF.isDisplayed()){
					tRejectRemarks.sendKeys("Draft rejected automation"+sDate);
					rejectGRNBtn.click();
					Thread.sleep(Constant.Min_Sleep);
	...
	public boolean validateDCDropDwn(int iRowNo, String sheet, String sInvoice){
		...
		try{
			...
			if(Constant.isElementPresent(By.xpath("//label[contains(text(),'"+sInvoice+"')]")) == false){
				flag = true;
			}
		}catch(Exception e){
			...
			if(grnDraft_EditPage.getText().contains(sDraftNo)){
				...
				if(materialUsagePopup.isDisplayed()){
					...
					if(Constant.driver.getPageSource().contains("No Records Found!")){
						...
						for(int i=0;i<materialUsageMaterils.size();i++){
							if(materialUsageMaterils.get(i).getText().contains(sMaterialList[0].trim())){
								flag = true;
							}
						}
					}
