...
//				dateSelect.click();
				...
				editPayStruct.click();
				Thread.sleep(Constant.Min_Sleep);
				for(int i=0;i<payStructDetailsList.size();i++){
					payStructDetailsList.get(i).sendKeys(Keys.CONTROL+"a");
					payStructDetailsList.get(i).sendKeys(Keys.DELETE);
					payStructDetailsList.get(i).sendKeys("500");
				}
				closePayStruc.click();
				Thread.sleep(Constant.Min_Sleep);
		...
		try{
			...
			if(empFirstName.isDisplayed()){
				...
				for(int i=0;i<payStructDetailsList.size();i++){
					payStructDetailsList.get(i).sendKeys(Keys.CONTROL+"a");
					payStructDetailsList.get(i).sendKeys(Keys.DELETE);
					payStructDetailsList.get(i).sendKeys("250");
				}
				closePayStruc.click();
				Thread.sleep(Constant.Min_Sleep);
				empPlaceOfWrk.clear();
